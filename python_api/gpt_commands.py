...
def ai_command(command):
    ...
    messages.append(
        {
            "role": "system",
            "content": "You are segmenting commands into functions. You will be passed a command from the user. Identify the function that the command belongs to. You must pick a command",
        }
    )
    messages.append(
        {
            "role": "user",
            "content": "You must pick the command you think is right. \n"
            + "Command: \n"
            + "### \n"
            + command,
        }
