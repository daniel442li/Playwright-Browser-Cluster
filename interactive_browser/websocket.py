...
async def handle_click(websocket, session, data):
    ...
    if coordinates and "x" in coordinates and "y" in coordinates:
        ...
        await websocket.send_text(
            "Error: Missing/invalid coordinates for click action."
        )
...
async def handle_hover(websocket, session, data):
    ...
    if coordinates and "x" in coordinates and "y" in coordinates:
        ...
        await websocket.send_text(
            f"Hover action performed at: {coordinates['x']}, {coordinates['y']}"
        )
    else:
        await websocket.send_text(
            "Error: Missing/invalid coordinates for hover action."
        )
...
async def handle_insert_bounding(websocket, session, data):
    if "query" in data:
        ...
        await websocket.send_text(
            f"Accessibility tree retrieved for query {data['query']}: {result}"
        )
    else:
